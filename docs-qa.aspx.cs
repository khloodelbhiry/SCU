...
public partial class docs_qa : System.Web.UI.Page
{
    ...
    private void BindControls()
    {
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            var query5 = (from q in db.FileDocumentKeywords
                          select new
                          {
                              q.keyword
                          }).Distinct();
            ...
            {
                dt.Rows.Add(count, i.keyword);
                count++;
            }
            ddlKeywords.DataSource = dt;
            ddlKeywords.DataTextField = "keyword";
    ...
    private void BindDocment(FileDocument query)
    {
        ...
        var refe = from r in query.FileDocumentReferences
                   select new
                   {
                       id = r.id,
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        if (Page.IsValid)
        {
            if (!File.Exists(Server.MapPath("Docs/" + txtDocCode.Text + ".pdf")))
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Startup", "<script language='javascript'> alert('يرجى التأكد من ارفاق ملف الوثيقة اولاً');</script>", false);
                return;
            }
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    var keyords = db.FileDocumentKeywords.Where(x => x.fileDocumentId == q.id);
                    db.FileDocumentKeywords.DeleteAllOnSubmit(keyords);
                    ...
                    db.FileDocumentReferences.DeleteAllOnSubmit(references);
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (q != null)
                    {
                        ...
                        db.SubmitChanges();
                        hdfFile.Value = null;
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Startup", "<script language='javascript'> alert('تمت مراجعة وثائق الملف');</script>", false);
                        GetNextDocument();
                    }
                }
            }
    ...
    private void ClearControls()
    {
        ...
        gdvReferences.DataSource = DtReference;
        gdvReferences.DataBind();
        txtFileReference.Text = txtFileTitle.Text = txtDocCode.Text = txtDocumentDateDay.Text = txtDocumentDateMonth.Text = txtDocumentDateYear.Text = txtNotes.Text = txtReference.Text = txtTitle.Text = string.Empty;
