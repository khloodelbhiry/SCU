...
public partial class DocumentTypes : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lnkHistory.HRef = "log.aspx?t=" + EncryptString.Encrypt("DocTypes") + "&p=" + Request.QueryString["id"];
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) &&
                        (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath));
            ...
            BindDDL();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
    ...
    private void BindData()
    {
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                var query = from t in db.DocTypes
        ...
        if (Page.IsValid)
        {
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    {
                        DocType u =
                            db.DocTypes.FirstOrDefault(x => x.id.Equals(int.Parse(ViewState["ID"].ToString())));
                        if (u != null)
                        {
                            u.name = txtName.Text.Trim();
                            db.SubmitChanges();
                            LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                        }
                    }
                    else
                    {
                        DocType u = new DocType();
                        u.name = txtName.Text.Trim();
                        u.statusId = (int)StatusEnum.UnderApprrove;
                        u.projectId = int.Parse(EncryptString.Decrypt(Request.QueryString["id"]));
                        db.DocTypes.InsertOnSubmit(u);
                        db.SubmitChanges();
                        LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Add).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                    }
    ...
    protected void lnkNewDocType_Click(object sender, EventArgs e)
    {
        ClearDocumentTypeControls();
    }
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && p.Edit.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            {
                ViewState["ID"] = e.CommandArgument;
                DocType u = db.DocTypes.FirstOrDefault(x => x.id.Equals(int.Parse(e.CommandArgument.ToString())));
                if (u != null)
                    db.DocTypes.DeleteOnSubmit(u);
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Delete).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                BindData();
    ...
    protected void btnNewDocType_Click(object sender, EventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && p.Add.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            DocType c = db.DocTypes.Where(x => x.id == int.Parse(ViewState["ID"].ToString())).FirstOrDefault();
            try
            {
                c.statusId= (int)StatusEnum.Freezed;
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectDocTypesPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
