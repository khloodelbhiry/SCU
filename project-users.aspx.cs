...
public partial class project_users : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lnkHistory.HRef = "log.aspx?t=" + EncryptString.Encrypt("Users") + "&p=" + Request.QueryString["id"];
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) &&
                    (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    if (Request.QueryString["id"] != null && Request.QueryString["g"] != null)
                    {
                        ...
                        {
                            var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath));
            ...
            BindDDL();
            BindUnitStructures();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
    private void BindUnitStructures()
    {
        tvMenu.Nodes.Clear();
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            var menu = (from m in db.UnitStructures
                        where m.parentId == 0
                        && m.governmentalEntityId == int.Parse(EncryptString.Decrypt(Request.QueryString["g"]))
                        select new
                        {
                            m.id,
                            m.name,
    ...
    private void BindDDL()
    {
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            ddlUnit.DataSource = query1;
            ddlUnit.DataTextField = "name";
            ddlUnit.DataValueField = "id";
            ddlUnit.DataBind();
            ddlUnit.Items.Insert(0, new ListItem("-- اختر --", "0"));

            ddlUnitSrc.DataSource = query1;
            ddlUnitSrc.DataTextField = "name";
            ddlUnitSrc.DataValueField = "id";
            ddlUnitSrc.DataBind();
            ddlUnitSrc.Items.Insert(0, new ListItem("-- اختر --", "0"));
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                var query = (from b in db.Users
                             where b.governmentalEntityId == int.Parse(EncryptString.Decrypt(Request.QueryString["g"]))
                             select new
                             {
                                 b.id,
                                 b.fullName,
                                 ...
                                 b.statusId,
                                 Group = b.Group.name,
                                 UnitStructure = b.UnitStructure.name,
                ...
                if (ddlUnitSrc.SelectedValue != "0")
                    query = query.Where(x => x.unitStructureId.Equals(int.Parse(ddlUnitSrc.SelectedValue)));
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Show.Equals(true)))
        {
    ...
    protected void btnClearSearch_Click(object sender, EventArgs e)
    {
        ...
        ddlStatusSrc.SelectedValue=ddlUnitSrc.SelectedValue =  "0";
        BindData();
    }
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                if (ViewState["id"] != null)
                {
                    ...
                    q.unitStructureId = ddlUnit.SelectedValue != "0" ? (int?)int.Parse(ddlUnit.SelectedValue) : null;
                    q.groupId = ddlGroup.SelectedValue != "0" ? (int?)int.Parse(ddlGroup.SelectedValue) : null;
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                }
                else
                {
                    User q = new User();
                    ...
                    q.mobile = txtMobileReg.Text.Trim();
                    q.governmentalEntityId = int.Parse(EncryptString.Decrypt(Request.QueryString["g"]));
                    q.unitStructureId = ddlUnit.SelectedValue != "0" ? (int?)int.Parse(ddlUnit.SelectedValue) : null;
                    q.groupId = ddlGroup.SelectedValue != "0" ? (int?)int.Parse(ddlGroup.SelectedValue) : null;
                    q.statusId = (int)StatusEnum.UnderApprrove;
                    db.Users.InsertOnSubmit(q);
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                }
    ...
    private void ClearControls()
    {
        ...
        ddlUnit.SelectedValue = ddlGroup.SelectedValue = "0";
        ViewState["id"] = null;
    }
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Add.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Delete).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                BindData();
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Edit.Equals(true)))
        {
        ...
        if (ViewState["id"] != null)
        {
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    ddlUnit.SelectedValue = query.unitStructureId != null ? query.unitStructureId.ToString() : "0";
                ...
                foreach (GridViewRow row in gdvData.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (chkRow.Checked)
                        {
                            ...
                            db.SubmitChanges();
                            LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                        }
                    }
                }
                if (checkedRows == 0)
    ...
    protected void btnAssign_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in gdvData.Rows)
        {
            if (row.RowType == DataControlRowType.DataRow)
            {
                HtmlInputCheckBox chkRow = (row.Cells[0].FindControl("chkUnitRow") as HtmlInputCheckBox);
                if (chkRow.Checked)
                {
                    checkedRows += 1;
                }
    ...
    protected void lnkCloseUnit_Click(object sender, EventArgs e)
    {
        mpeUnit.Hide();
        BindData();
    }
    ...
    protected void btnAssigned_Click(object sender, EventArgs e)
    {
        if (ViewState["selectedid"] != null)
        {
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    foreach (GridViewRow row in gdvData.Rows)
                    {
                        if (row.RowType == DataControlRowType.DataRow)
                        {
                            HtmlInputCheckBox chkUnitRow = (row.Cells[0].FindControl("chkUnitRow") as HtmlInputCheckBox);
                            if (chkUnitRow.Checked)
                            {
                                checkedRows += 1;
                                HiddenField hdfID = (row.Cells[0].FindControl("hdfID") as HiddenField);
                                User u = db.Users.FirstOrDefault(x => x.id == int.Parse(hdfID.Value));
                                u.unitStructureId = int.Parse(ViewState["selectedid"].ToString());
                                db.SubmitChanges();
                                LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                            }
                        }
            ...
            ScriptManager.RegisterStartupScript(this, GetType(), "alertUser",
               "alert('يرجى اختيار الأدارة اولا');", true);
    }
    protected void tvMenu_SelectedNodeChanged(object sender, EventArgs e)
    {
        ViewState["selectedid"] = tvMenu.SelectedNode.Value;
        lblUserUnit.Text = tvMenu.SelectedNode.Text;
        mpeUnit.Show();
    }

    protected void btnFreeze_Click(object sender, EventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectUsersPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                if (c.groupId != null)
                {
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
