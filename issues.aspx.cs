...
public partial class issues: System.Web.UI.Page
{
        ...
        if (!IsPostBack)
        {
            ...
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.IssuesPath) &&
                        (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.IssuesPath));
            ...
            BindDDL();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Add.Equals(true)))
        {
            ...
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    if (ViewState["id"] == null)
                    {
                        ...
                        db.SubmitChanges();
                        LogWriter.LogWrite("Issues", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                    }
                    else
                    {
                        ...
                        db.SubmitChanges();
                        LogWriter.LogWrite("Issues", ((int)ActivitiesEnum.Update).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                    }
                    ClearControls();
                    BindData();
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Edit.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Issues", ((int)ActivitiesEnum.Delete).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                BindData();
            }
            catch (Exception ex)
            {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Show.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Issues", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                FillControls();
                BindData();
            }
            catch (Exception ex)
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.IssuesPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Issues", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "اغلاق المشكلة");
                FillControls();
                BindData();
            }
            catch (Exception ex)
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                if (ViewState["NoteID"] == null)
                {
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("IssueNotes", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "اضافة ملاحظة (المشكلات)");
                }
                else
                {
                    ...
                    q.note = txtNotes.Text;
                    db.SubmitChanges();
                    LogWriter.LogWrite("IssueNotes", ((int)ActivitiesEnum.Update).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "تعديل ملاحظة (المشكلات)");
                }
                ClearNotesControls();
                mpeNotes.Show();
                BindNotes();
    ...
    protected void btnApproveNotes_Click(object sender, EventArgs e)
    {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("IssueNotes", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "اعتماد ملاحظة (المشكلات)");
                ClearNotesControls();
                BindNotes();
                mpeNotes.Show();
            }
    ...
    protected void btnFreezeNotes_Click(object sender, EventArgs e)
    {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("IssueNotes", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "تجميد ملاحظة (المشكلات)");
                ClearNotesControls();
                BindNotes();
                mpeNotes.Show();
            }
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                if (fuNotesAttachments.HasFile)
                {
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("NoteAttachments", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "اضافة مرفق على ملاحظة (المشكلات)");
    ...
    protected void btnDeleteNoteAttachment_Command(object sender, CommandEventArgs e)
    {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            db.SubmitChanges();
            LogWriter.LogWrite("NoteAttachments", ((int)ActivitiesEnum.Delete).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "حذف مرفق لملاحظة (المشكلات)");
            BindNotesAttachments();
            mpeNotesAttachments.Show();
    ...
    protected void btnDeleteIssueAttachment_Command(object sender, CommandEventArgs e)
    {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("IssueAttachments", ((int)ActivitiesEnum.Delete).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "حذف مرفق (المشكلات)");
                BindIssueAttachments();
                BindData();
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                if (fuIssueAttachment.HasFile)
                {
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("IssueAttachments", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, "اضافة مرفق (المشكلات)");
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Startup", "<script language='javascript'> alert('تم الحفظ بنجاح');</script>", false);
                    BindIssueAttachments();
                    BindData();
                    mpeIssueAttachment.Show();
