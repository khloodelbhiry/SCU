...
public partial class project_users : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lnkHistory.HRef = "log.aspx?t=" + EncryptString.Encrypt("Users") + "&c=" + Request.QueryString["id"];
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) &&
                    (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    if (Request.QueryString["id"] != null)
                    {
                        ...
                        {
                            var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath));
            ...
            BindDDL();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
    private void BindDDL()
    {
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            var query1 = from q in db.Projects
            ...
            ddlProject.DataSource = query1;
            ddlProject.DataTextField = "name";
            ddlProject.DataValueField = "id";
            ddlProject.DataBind();
            ddlProject.Items.Insert(0, new ListItem("-- اختر --", "0"));

            ddlProjectAssign.DataSource = query1;
            ddlProjectAssign.DataTextField = "name";
            ddlProjectAssign.DataValueField = "id";
            ddlProjectAssign.DataBind();
            ddlProjectAssign.Items.Insert(0, new ListItem("-- اختر --", "0"));

            ddlProjectSrc.DataSource = query1;
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Show.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                if (ViewState["id"] != null)
                {
                    ...
                    q.projectId = ddlProject.SelectedValue != "0" ? (int?)int.Parse(ddlProject.SelectedValue) : null;
                    q.groupId = ddlGroup.SelectedValue != "0" ? (int?)int.Parse(ddlGroup.SelectedValue) : null;
                    //q.governmentalEntityId = ddlProject.SelectedValue != "0" ? (int?)(db.Projects.FirstOrDefault(x => x.id == int.Parse(ddlProject.SelectedValue)).governmentalEntityId) : null;
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]),string.Empty, string.Empty);
                }
                else
                {
                    User q = new User();
                    q.companyId = int.Parse(EncryptString.Decrypt(Request.QueryString["id"]));
                    q.fullName = txtFullName.Text.Trim();
                    q.email = txtElecM.Text.Trim();
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Add).ToString(), q.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
                }
    ...
    private void ClearControls()
    {
        ...
        ddlProject.SelectedValue = ddlGroup.SelectedValue = "0";
        ViewState["id"] = null;
    }
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Add.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Delete).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
                BindData();
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Edit.Equals(true)))
        {
        ...
        if (ViewState["id"] != null)
        {
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    ddlProject.SelectedValue = query.projectId != null ? query.projectId.ToString() : "0";
            ...
            try
            {
                ...
                foreach (GridViewRow row in gdvData.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        {
                            checkedRows += 1;
                            HiddenField hdfID = (row.Cells[1].FindControl("hdfID") as HiddenField);
                            User u = db.Users.FirstOrDefault(x => x.id == int.Parse(hdfID.Value));
                            u.hasWorkLicense = true;
                            db.SubmitChanges(); 
                            LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
                        }
                    }
                }
                if (checkedRows == 0)
    ...
    protected void lnkCloseProject_Click(object sender, EventArgs e)
    {
        mpeProject.Hide();
        BindData();
    }
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges(); 
                LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.CompanyUsersPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                if (c.groupId != null)
                {
                    ...
                    db.SubmitChanges();
                    LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
                ...
                foreach (GridViewRow row in gdvData.Rows)
                {
                    if (row.RowType == DataControlRowType.DataRow)
                    {
                        ...
                        if (chkProjectRow.Checked)
                        {
                            ...
                            User u = db.Users.FirstOrDefault(x => x.id == int.Parse(hdfID.Value));
                            u.projectId = ddlProjectAssign.SelectedValue != "0" ? (int?)int.Parse(ddlProjectAssign.SelectedValue) : null;
                            u.governmentalEntityId = ddlProjectAssign.SelectedValue != "0" ? (int?)(db.Projects.FirstOrDefault(x => x.id == int.Parse(ddlProjectAssign.SelectedValue)).governmentalEntityId) : null;
                            db.SubmitChanges();
                            LogWriter.LogWrite("Users", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty, string.Empty);
                        }
                    }
                }
                ddlProjectAssign.SelectedValue = "0";
                BindData();
                ScriptManager.RegisterStartupScript(this, GetType(), "alertUser",
                   "alert('تم الحفظ بنجاح');", true);
            }
