...
public partial class DocumentTypes : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lnkHistory.HRef = "log.aspx?t=" + EncryptString.Encrypt("DocTypes");
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.DocTypesPath) &&
                        (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath));
            ...
            BindDDL();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                var query = from t in db.DocTypes
                            where t.statusId == (int)StatusEnum.Approved
                            select new
                            {
                                t.id,
                                t.name,
                ...
                try
                {
                    if (ViewState["ID"] != null)
                    {
                        ...
                        if (u != null)
                        {
                            ...
                            db.SubmitChanges();
                            LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                        }
                    }
                    else
                    {
                        ...
                        db.SubmitChanges();
                        LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Add).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                    }
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && p.Edit.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Delete).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
                BindData();
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && p.Add.Equals(true)))
        {
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty);
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.DocTypesPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("DocTypes", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, string.Empty, string.Empty); 
