...
public partial class DocumentTypes : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            lnkHistory.HRef = "log.aspx?t=" + EncryptString.Encrypt("BusinessReferences") + "&p=" + Request.QueryString["id"];
            if (Session["User"] != null && Session["User"].ToString() != string.Empty)
            {
                if (UserPermissions.Any(
                    p =>
                        p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) &&
                        (p.Show.Equals(true) || p.Add.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                {
                    var per = UserPermissions.FirstOrDefault(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath));
            ...
            BindDDL();
            if (UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && (p.Show.Equals(true) || p.Edit.Equals(true) || p.Delete.Equals(true) || p.Approve.Equals(true) || p.Freze.Equals(true))))
                BindData();
        }
    }
    ...
    private void BindData()
    {
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                var query = from t in db.BusinessReferences
                            select new
                            {
                                t.id,
                                t.name,
    ...
    private void ClearDocumentTypeControls()
    {
        ...
        if (Page.IsValid)
        {
            using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
            {
                try
                {
                    ...
                    {
                        BusinessReference u =
                            db.BusinessReferences.FirstOrDefault(x => x.id.Equals(int.Parse(ViewState["ID"].ToString())));
                        if (u != null)
                        {
                            u.name = txtName.Text.Trim();
                            db.SubmitChanges();
                            LogWriter.LogWrite("BusinessReferences", ((int)ActivitiesEnum.Update).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                        }
                    }
                    else
                    {
                        BusinessReference u = new BusinessReference();
                        u.name = txtName.Text.Trim();
                        u.statusId = (int)StatusEnum.UnderApprrove;
                        db.BusinessReferences.InsertOnSubmit(u);
                        db.SubmitChanges();
                        LogWriter.LogWrite("BusinessReferences", ((int)ActivitiesEnum.Add).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                    }
    ...
    protected void lnkNewBusinessReference_Click(object sender, EventArgs e)
    {
        ClearDocumentTypeControls();
    }
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && p.Edit.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            try
            {
                ...
                var u = (from t in db.BusinessReferences
                ...
                if (u != null)
                {
                    ...
                    btnFreeze.Visible = u.statusId != (int)StatusEnum.Freezed && u.statusId == null;
                }
            }
            catch (Exception ex)
            {
        ...
        mpeDocumentType.Show();
    }
    protected void lnkDelete_Command(object sender, CommandEventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && p.Delete.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            {
                ViewState["ID"] = e.CommandArgument;
                BusinessReference u = db.BusinessReferences.FirstOrDefault(x => x.id.Equals(int.Parse(e.CommandArgument.ToString())));
                if (u != null)
                    db.BusinessReferences.DeleteOnSubmit(u);
                db.SubmitChanges();
                LogWriter.LogWrite("BusinessReferences", ((int)ActivitiesEnum.Delete).ToString(), u.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
                BindData();
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && p.Add.Equals(true)))
        {
    ...
    {
        ClearDocumentTypeControls();
        mpeDocumentType.Hide();
    }

    protected void btnFreeze_Click(object sender, EventArgs e)
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && p.Freze.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            BusinessReference c = db.BusinessReferences.Where(x => x.id == int.Parse(ViewState["ID"].ToString())).FirstOrDefault();
            try
            {
                c.statusId = (int)StatusEnum.Freezed;
                db.SubmitChanges();
                LogWriter.LogWrite("BusinessReferences", ((int)ActivitiesEnum.Freze).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
    ...
    {
        if (!UserPermissions.Any(p => p.PageUrl.ToLower().Equals(Common.ProjectBusinessReferencesPath) && p.Approve.Equals(true)))
        {
        ...
        using (SCU_OneTrackDataContext db = new SCU_OneTrackDataContext())
        {
            ...
            try
            {
                ...
                db.SubmitChanges();
                LogWriter.LogWrite("BusinessReferences", ((int)ActivitiesEnum.Approve).ToString(), c.id.ToString(), DateTime.Now.ToString(), UserDetails.DeSerializeUserDetails(Session["User"].ToString()).ID.ToString(), string.Empty, string.Empty, string.Empty, string.Empty, EncryptString.Decrypt(Request.QueryString["id"]), string.Empty);
